...
battery_management::battery_management(int batteryVoltDivPin) 
    : _batteryVoltDivPin(batteryVoltDivPin), _lowBatteryThreshold(11.2),
      _criticalLevel(20), _lowLevel(30), _mediumLevel(50) {
    pinMode(_batteryVoltDivPin, INPUT);
}

void battery_management::readBatteryVoltage() {
    int battAnalogVal = analogRead(_batteryVoltDivPin);
    float battTrueVal = (map(battAnalogVal, 0, 1023, 0, 500)) / 100.0 * 2.5; // volt divider rescaling
    _batteryVoltage = battTrueVal;
    _batteryCapacity = _mapFloat(battTrueVal, 11.2, 12.6, 20, 100);
}

float battery_management::getBatteryVoltage() {
    return _batteryVoltage;
}

int battery_management::getBatteryCapacity() {
    return _batteryCapacity;
}

bool battery_management::isLowBattery() {//returns true or false
    return _batteryVoltage < _lowBatteryThreshold;
}

void battery_management::setLowBatteryThreshold(float threshold) {
    _lowBatteryThreshold = threshold;
}

void battery_management::setBatteryWarningLevels(int critical, int low, int medium) {
    _criticalLevel = critical;
    _lowLevel = low;
    _mediumLevel = medium;
}

float battery_management::_mapFloat(float x, float in_min, float in_max, float out_min, float out_max) {
