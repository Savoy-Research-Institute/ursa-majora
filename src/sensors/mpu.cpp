...
const int mpu_transistor_pin = 2; // Adjust as needed
bool is_mpu_on = false;

float acc_x, acc_y, acc_z;
float gyro_x, gyro_y, gyro_z;
float acc_err_x, acc_err_y, gyro_err_x, gyro_err_y, gyro_err_z;
float gyroAngleX, gyroAngleY, roll, pitch, yaw;
...
void read_mpu_gyroscope()
{
    ...
    while (is_mpu_on)
    {
        ...
        gyro_y_raw = (Wire.read() << 8 | Wire.read());// For a 250deg/s range we have to divide first the raw value by 131.0,
        gyro_z_raw = (Wire.read() << 8 | Wire.read());
...
{ /* Self-test allows for the testing of the mechanical and electrical portions of the sensors. The self-test for each
    ...
    // Step 1: Check communication by reading the WHO_AM_I register
    if (read_who_am_i() != MPU6050_ID)
    {
        Serial.println("MPU6050 communication failed.");
        return;
    }

    // Step 2: Capture baseline readings
    Vector3D baseline_accel = read_accelerometer();
    Vector3D baseline_gyro = read_gyroscope();

    // Step 3: Enable self-test mode
    enable_self_test(); // Writes to ACCEL_CONFIG and GYRO_CONFIG registers

    // Step 4: Capture self-test readings
    Vector3D self_test_accel = read_accelerometer();
    Vector3D self_test_gyro = read_gyroscope();

    // Step 5: Compare baseline and self-test results
    if (!compare_self_test(baseline_accel, self_test_accel) ||
        !compare_self_test(baseline_gyro, self_test_gyro))
    {
        Serial.println("MPU6050 self-test failed.");
    }
    else
    {
        Serial.println("MPU6050 self-test passed.");
    }

    // Step 6: Disable self-test mode and continue normal operation
    disable_self_test();
}
...
place at a certain memory place I can always go to fetch it...*/
