...
float pid_d_gain_roll = 8;                //Gain setting for the roll D-controller (15)//fine tune these, procedure at README.md
int pid_max_roll = 400;                    //Maximum output of the PID-controller (+/-)

float pid_p_gain_pitch = pid_p_gain_roll;  //Gain setting for the pitch P-controller.//Pitch same as roll for all controllers
...
//set at 0 because yaw has props drag already, I'll test more on these
...
int  LED1=22, LED2=23, LED3=24,LED4=24, LED5=26, LED6=27;//WARNING LEDs, peripheral feedback system
int  LEDerror=1;
int low_battery_voltage=4.44;//Percieved voltage 
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Setup routine
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void setup(){
  ...
  rawSTr=attachInterrupt(13, CHANGE);
  ...
  //DDRH |= B00100000;//h5,pin8 OUTPUT, LED, currently not needed
  ...
  Wire.write(0x1B);                  // Talk to the register 1B
  Wire.write(0x00);                  // Make reset to the mpu  - place a 0 into the 1B register
...
void batteryCapacity() {
  //Battery management, Objectives, to know battery low, compensation of ESCs Battery capacity
  /*Read battery via voltage divider A0, key in dividing variable, 
  find exact voltage, */

//12.6v down to 5.04v{exactly 2.5 times division}
  int rawValue = analogRead(analogPin); // Read the raw ADC value (0-1023)

  // Map the raw value to a 0-5V range (0-255)
  float mappedValue = map(rawValue, 0, 1023, 0, 5);

  // Calculate the voltage (0-5V) based on the mapped value
  float realVoltage =(rawValue) * 2.5; // (5V / 255)
  float batteryCapacity=(realVoltage)/221;
  //Y = map(X, 20, 80, 0, 100);
  ...
  //Serial.println(realVoltage); // Display with two decimal places
  // Serial.println(" V");
  //Serial.println(batteryCapacity);
}
//the max is around 11.5v i think that is 1023, at this point, the drone reads voltage from a cell, assumes that is same to every cell, before the total voltage drops to a readable point,
//battery low alarm LED sequences
//Percieved voltage above threshhold(11.5-12.6v)
